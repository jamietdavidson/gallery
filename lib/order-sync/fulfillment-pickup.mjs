...
function pickupNotesIndicateScheduleFailed(notes) {
  return /schedule_failed:/i.test(String(notes ?? ''));
}

function isPickupScheduleFailed(record) {
  const pf = AIRTABLE.pickupFields;
  const fields = record?.fields ?? {};
  return (
    fields[pf.status] === AIRTABLE.pickupStatus.failed ||
    pickupNotesIndicateScheduleFailed(fields[pf.notes])
  );
}
...
function isPickupClosed(record) {
  ...
  return (
    status === AIRTABLE.pickupStatus.confirmed || isPickupScheduleFailed(record)
  );
}
...
function sortPickupsByWhen(records) {
  ...
  return [...records].sort((left, right) => {
    const leftAt =
      parseAirtableDate(left.fields?.[pf.when])?.getTime() ?? Infinity;
    const rightAt =
      parseAirtableDate(right.fields?.[pf.when])?.getTime() ?? Infinity;
...
export async function listUpcomingPickups($) {
  const pf = AIRTABLE.pickupFields;
  const formula = `AND(IS_AFTER({${pf.when}}, NOW()), {${pf.status}} != "${AIRTABLE.pickupStatus.confirmed}", {${pf.status}} != "${AIRTABLE.pickupStatus.failed}")`;
...
function appendEasypostPickupNote(existingNotes, easypostPickup) {
  const base = stripScheduleErrorNotes(String(existingNotes ?? '').trim());
  const lines = [
    base,
    `easypost_pickup:${easypostPickup.easypostPickupId}`,
    easypostPickup.confirmation
      ? `confirmation:${easypostPickup.confirmation}`
      : null,
...
function stripScheduleErrorNotes(notes) {
  return notes
    .split('\n')
    .filter((line) => !line.startsWith('schedule_error:'))
    .join('\n')
    .trim();
}

function appendScheduleErrorNote(existingNotes, error) {
  const base = stripScheduleErrorNotes(String(existingNotes ?? '').trim());
  const line = `schedule_error:${new Date().toISOString()}:${error}`;
  return [base, line].filter(Boolean).join('\n');
}

function appendScheduleFailedMarker(existingNotes) {
  const base = String(existingNotes ?? '')
    .split('\n')
    .filter((line) => !line.startsWith('schedule_failed:'))
    .join('\n')
    .trim();
  const line = `schedule_failed:${new Date().toISOString()}`;
  return [base, line].filter(Boolean).join('\n');
}

async function markPickupScheduleFailed($, pickupRecordId, existingNotes, error) {
  const pf = AIRTABLE.pickupFields;
  const notes = appendScheduleFailedMarker(
    appendScheduleErrorNote(existingNotes, error),
  );

  try {
    await updatePickupRecord($, pickupRecordId, {
      [pf.status]: AIRTABLE.pickupStatus.failed,
      [pf.notes]: notes,
    });
  } catch (err) {
    if (!String(err.message ?? '').includes('INVALID_MULTIPLE_CHOICE_OPTIONS')) {
      throw err;
    }
    // Airtable Status field may not include Failed yet — notes marker still stops retries.
    await updatePickupRecord($, pickupRecordId, {[pf.notes]: notes});
  }
}

async function resolveLinkedFulfillments($, pickupRecordId, linkedIds) {
  const ids = [...new Set(linkedIds)].filter(Boolean);
  if (ids.length) {
    return getFulfillmentRecordsByIds($, ids);
  }

  return listFulfillmentsForPickup($, pickupRecordId);
}
...
export async function listPickupsNeedingSchedule($) {
  ...
  return sortPickupsByWhen(records).filter(
    (record) => !isPickupScheduleFailed(record),
  );
}
...
function labeledFulfillmentsForPickup(fulfillmentRecords) {
  const ff = AIRTABLE.fulfillmentFields;
  return fulfillmentRecords.filter((record) =>
    Boolean(record.fields?.[ff.shippingLabel]),
  );
}
...
export async function linkFulfillmentToPickup(fulfillmentRecordId) {
  ...
  return {
    ...
    pickupLabel:
      pickup.fields?.[AIRTABLE.pickupFields.notes] ??
      formatPickupName(when, PICKUP_SCHEDULE.timeZone),
...
export async function schedulePickupRecord(pickupRecordId) {
  ...
  if (isPickupScheduleFailed(pickup)) {
    return {
      action: 'skipped',
      pickupRecordId,
      reason:
        'Pickup scheduling previously failed — clear schedule_failed from Notes and set Status to Requested to retry',
    };
  }
  ...
  if (!when) {
    const error = 'Pickup row is missing a When date';
    await markPickupScheduleFailed($, pickupRecordId, fields[pf.notes], error);
    return {
      action: 'failed',
      pickupRecordId,
      error,
  ...
  const fulfillments = await resolveLinkedFulfillments(
    $,
    pickupRecordId,
    linkedIds,
  );
  ...
  if (!labeled.length) {
    const error =
      fulfillments.length > 0
        ? 'Linked fulfillments are missing shipping labels — set them to In Progress first'
        : 'No fulfillments are linked to this pickup row';
    await markPickupScheduleFailed($, pickupRecordId, fields[pf.notes], error);
    return {
      action: 'failed',
      pickupRecordId,
  ...
  const scheduled = easypostResults.filter(
    (entry) => entry.result?.action === 'scheduled',
  );
  ...
  const error =
    skipped
      ...
      .filter(Boolean)
      .join('; ') || 'EasyPost could not schedule a carrier pickup';

  await markPickupScheduleFailed($, pickupRecordId, existingNotes, error);
